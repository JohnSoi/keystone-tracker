...
async def user_delete(request: Request, user: UserModel = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> bool:
...
async def user_update(request: Request, user_data: UserPublicData, db: AsyncSession = Depends(get_db), user: UserModel = Depends(get_current_user)) -> UserPublicData:
